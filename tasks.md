# Project Tasks & Progress

## 🟢 Phase 0: Project Initialization
- [x] Create repository structure (README, .gitignore, tasks)
- [x] Define folder structure (`firmware`, `hardware`, `software`, `docs`, `resources`)
- [x] Finalize Bill of Materials (BOM) (ESP32-S3, MG996R Analog, 12V 10A PS)
- [x] Organize initial resources (Datasheets, 3D Models)

## 🔘 Phase 1: Mechanical & Hardware Design
- [ ] Base rotation design (SolidWorks)
- [ ] Link 1 & 2 design (SolidWorks)
- [ ] 1 DOF Gripper design
- [x] Electronics wiring schematic (Refer to `hardware/hardware_specifications.md`)


## 🔘 Phase 2: Firmware (ESP32)
- [ ] Basic Servo PWM Control
- [ ] Analog Feedback integration (Closed-loop)
- [ ] Current sensing for gripper (Force detection)
- [ ] Inverse Kinematics (IK) logic

## 🔘 Phase 3: Advanced Integration
- [ ] PC-side Serial Control (Python)
- [ ] Digital Twin (Visual representation)
- [ ] Image Processing (Object localization)
- [ ] LLM Chatbot integration

---
*Last Updated: 2026-02-01 (Updated Resources & Structure)*

